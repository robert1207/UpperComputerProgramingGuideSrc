...
#include "protocolfindingdevicemanager.h"
//#include "my_log.h"

namespace HYKT {

ProtocolFindingDeviceManager::ProtocolFindingDeviceManager()
{
    timer_p = new QTimer();
    connect(timer_p, &QTimer::timeout, this, &ProtocolFindingDeviceManager::Update);
}

ProtocolFindingDeviceManager::~ProtocolFindingDeviceManager()
{
    if(timer_p->isActive()) timer_p->stop();
    disconnect(timer_p, &QTimer::timeout, this, &ProtocolFindingDeviceManager::Update);
    delete timer_p;
}

void ProtocolFindingDeviceManager::StartFindingDevice() {
   ...
   // D << "ip conunt=" <<  ip_list.count();
    ...
    for (int a = 0; a < ip_list.count(); a ++) {
        ...
        connect(device_finder, &DeviceFinder::onFindingError,
                                this, &ProtocolFindingDeviceManager::onFoundDeviceError);
...
void ProtocolFindingDeviceManager::StopFindingDevice() {
    timer_p->stop();
...
void ProtocolFindingDeviceManager::Update() {
